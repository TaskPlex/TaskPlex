...
import { useCSSMinifier } from '../hooks/useCSSMinifier';

export const CSSMinifierScreen: React.FC = () => {
  const { t } = useTranslation();
  const [css, setCss] = useState('');

  const { mutate, data: result, isPending: loading, error, reset } = useCSSMinifier();
  ...
  const handleCopy = useCallback(() => {
    if (result?.minified_css) {
      navigator.clipboard.writeText(result.minified_css);
    }
  }, [result]);

  const handleDownload = useCallback(() => {
    if (result?.minified_css) {
      const blob = new Blob([result.minified_css], { type: 'text/css' });
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="text-center mb-10">
        ...
        <p className="text-gray-600 dark:text-gray-400">{t('cssMinifier.description')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <textarea
              ...
              placeholder={t('cssMinifier.cssPlaceholder')}
            ...
            <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
              {css.length} {t('cssMinifier.characters')}
            </p>
          </div>
          ...
          >
            {loading ? (
              <>
                ...
                {t('cssMinifier.minifying')}
              </>
            ) : (
              <>
                <Minimize2 className="w-5 h-5" />
                {t('cssMinifier.minifyBtn')}
              </>
            )}
          </button>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <textarea
              readOnly
              value={result?.minified_css || ''}
              placeholder={t('cssMinifier.minifiedPlaceholder')}
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">
                      {t('cssMinifier.originalSize')}:
                    </span>
                    <span className="ml-2 font-semibold text-gray-900 dark:text-white">
                      {result.original_size} {t('cssMinifier.bytes')}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">
                      {t('cssMinifier.minifiedSize')}:
                    </span>
                    <span className="ml-2 font-semibold text-gray-900 dark:text-white">
                      {result.minified_size} {t('cssMinifier.bytes')}
                    </span>
                  </div>
                  {result.compression_ratio !== undefined && (
                    <div className="col-span-2">
                      <span className="text-gray-600 dark:text-gray-400">
                        {t('cssMinifier.compressionRatio')}:
                      </span>
