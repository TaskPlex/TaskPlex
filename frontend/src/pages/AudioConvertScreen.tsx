...
import { useConvertAudio } from '../hooks/useAudio';
import { FileDropzone, ProcessButton, ErrorAlert, ResultCard, FormatSelector, QualitySelector } from '../components/ui';
...
export const AudioConvertScreen: React.FC = () => {
  ...
  const { mutate, data, isPending, isSuccess, isError, error, reset } = useConvertAudio();
  ...
  const handleSubmit = useCallback(() => {
    if (!file) {
      setLocalError(t('audioConvert.validation.fileRequired'));
      return;
    }
    setLocalError(null);
    mutate({ file, outputFormat: format, quality, bitrate });
  }, [file, format, quality, bitrate, mutate, t]);
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex flex-col gap-2 mb-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('audioConvert.title')}</h1>
        </div>
        <p className="text-gray-600 dark:text-gray-300">{t('audioConvert.description')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          <FileDropzone
            ...
            accept="audio/*"
            fileType="audio"
            labelKey="audioConvert.audioFile"
            dropLabelKey="audioConvert.dragDrop"
            color="pink"
            disabled={isPending}
          />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormatSelector
              ...
              value={format}
              onChange={setFormat}
              labelKey="audioConvert.outputFormat"
              disabled={isPending}
            />
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">{t('audioConvert.bitrateHint')}</p>
          </div>
          ...
          <ProcessButton
            ...
            loading={isPending}
            labelKey="audioConvert.convert"
            loadingLabelKey="audioConvert.converting"
            color="pink"
          />
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200">{t('audioConvert.result')}</h2>
          ...
          {isPending && (
            <div className="h-56 bg-gradient-to-br from-pink-50 to-purple-50 dark:from-pink-900/20 dark:to-purple-900/20 border-2 border-pink-200 dark:border-pink-800 rounded-xl flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-pink-700 dark:text-pink-300 font-medium">{t('audioConvert.converting')}</p>
            </div>
          )}
          ...
          {!isPending && (!data || !data.success) && (
            <div className="h-56 bg-gray-50 dark:bg-gray-800 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl flex flex-col items-center justify-center text-gray-400 dark:text-gray-500">
              ...
              <p className="text-sm">{t('audioConvert.previewPlaceholder')}</p>
            </div>
          )}
        </div>
      </div>
