...
export const JSONMinifierScreen: React.FC = () => {
  const { t } = useTranslation();
  const [json, setJson] = useState('');

  const { mutate, data: result, isPending: loading, error, reset } = useJSONMinifier();
  ...
  const handleCopy = useCallback(() => {
    if (result?.minified_json) {
      navigator.clipboard.writeText(result.minified_json);
    }
  }, [result]);

  const handleDownload = useCallback(() => {
    if (result?.minified_json) {
      const blob = new Blob([result.minified_json], { type: 'application/json' });
    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="text-center mb-10">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2 flex items-center justify-center gap-3">
          ...
          {t('jsonMinifier.title')}
        </h1>
        <p className="text-gray-600 dark:text-gray-400">{t('jsonMinifier.description')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <textarea
              ...
              placeholder={t('jsonMinifier.jsonPlaceholder')}
            ...
            <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
              {json.length} {t('jsonMinifier.characters')}
            </p>
          </div>
          ...
          >
            {loading ? (
              <>
                ...
                {t('jsonMinifier.minifying')}
              </>
            ) : (
              <>
                <Minimize2 className="w-5 h-5" />
                {t('jsonMinifier.minifyBtn')}
              </>
            )}
          </button>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <textarea
              readOnly
              value={result?.minified_json || ''}
              placeholder={t('jsonMinifier.minifiedPlaceholder')}
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">
                      {t('jsonMinifier.originalSize')}:
                    </span>
                    <span className="ml-2 font-semibold text-gray-900 dark:text-white">
                      {result.original_length} {t('jsonMinifier.bytes')}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">
                      {t('jsonMinifier.minifiedSize')}:
                    </span>
                    <span className="ml-2 font-semibold text-gray-900 dark:text-white">
                      {result.minified_length} {t('jsonMinifier.bytes')}
                    </span>
                  </div>
                  {result.compression_ratio !== undefined && (
                    <div className="col-span-2">
                      <span className="text-gray-600 dark:text-gray-400">
                        {t('jsonMinifier.compressionRatio')}:
                      </span>
