...
import { Code, Copy, Download, RefreshCw, Code2 } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { useXMLFormatter } from '../hooks/useXMLFormatter';

export const XMLFormatterScreen: React.FC = () => {
  const { t } = useTranslation();
  const [xml, setXml] = useState('');
  const [indentSize, setIndentSize] = useState(2);

  const { mutate, data: result, isPending: loading, error, reset } = useXMLFormatter();

  const handleFormat = useCallback(() => {
    if (!xml.trim()) return;
    mutate({ xml, indentSize });
  }, [xml, indentSize, mutate]);
  ...
  const handleDownload = useCallback(() => {
    if (result?.result) {
      const blob = new Blob([result.result], { type: 'application/xml' });
    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="text-center mb-10">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2 flex items-center justify-center gap-3">
          <Code className="text-yellow-600 dark:text-yellow-400" size={32} />
          {t('xmlFormatter.title')}
        </h1>
        <p className="text-gray-600 dark:text-gray-400">{t('xmlFormatter.description')}</p>
      </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <textarea
              ...
              placeholder={t('xmlFormatter.xmlPlaceholder')}
            ...
            <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
              {xml.length} {t('xmlFormatter.characters')}
            </p>
          </div>
            ...
            >
              {loading ? (
                <>
                  ...
                  {t('xmlFormatter.formatting')}
                </>
              ) : (
                <>
                  <Code2 className="w-5 h-5" />
                  {t('xmlFormatter.formatBtn')}
                </>
              )}
            </button>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <textarea
              readOnly
              value={result?.result || ''}
              placeholder={t('xmlFormatter.formattedPlaceholder')}
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">
                      {t('xmlFormatter.originalSize')}:
                    </span>
                    <span className="ml-2 font-semibold text-gray-900 dark:text-white">
                      {result.original_size} {t('xmlFormatter.bytes')}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">
                      {t('xmlFormatter.resultSize')}:
                    </span>
                    <span className="ml-2 font-semibold text-gray-900 dark:text-white">
                      {result.result_size} {t('xmlFormatter.bytes')}
                    </span>
                  </div>
                </div>
              </div>
