...
export const HTMLFormatterScreen: React.FC = () => {
  const { t } = useTranslation();
  const [html, setHtml] = useState('');
  const [indentSize, setIndentSize] = useState(2);
  const [indentChar, setIndentChar] = useState(' ');

  const { mutate, data: result, isPending: loading, error, reset } = useHTMLFormatter();

  const handleFormat = useCallback(() => {
    if (!html.trim()) return;
    mutate({ html, indentSize, indentChar });
  }, [html, indentSize, indentChar, mutate]);

  const handleCopy = useCallback(() => {
    if (result?.formatted_html) {
      navigator.clipboard.writeText(result.formatted_html);
    }
  }, [result]);

  const handleDownload = useCallback(() => {
    if (result?.formatted_html) {
      const blob = new Blob([result.formatted_html], { type: 'text/html' });
    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="text-center mb-10">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2 flex items-center justify-center gap-3">
          ...
          {t('htmlFormatter.title')}
        </h1>
        <p className="text-gray-600 dark:text-gray-400">{t('htmlFormatter.description')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <textarea
              ...
              placeholder={t('htmlFormatter.htmlPlaceholder')}
            ...
            <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
              {html.length} {t('htmlFormatter.characters')}
            </p>
          </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full p-3 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-yellow-500 dark:focus:ring-yellow-400 focus:border-transparent text-gray-900 dark:text-white"
                >
                  <option value=" ">{t('htmlFormatter.space')}</option>
                  <option value="\t">{t('htmlFormatter.tab')}</option>
                </select>
              </div>
            </div>
            ...
            >
              {loading ? (
                <>
                  ...
                  {t('htmlFormatter.formatting')}
                </>
              ) : (
                <>
                  <FileCode className="w-5 h-5" />
                  {t('htmlFormatter.formatBtn')}
                </>
              )}
            </button>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <textarea
              readOnly
              value={result?.formatted_html || ''}
              placeholder={t('htmlFormatter.formattedPlaceholder')}
            ...
            {result?.formatted_html && (
              <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                {result.formatted_length} {t('htmlFormatter.characters')}
