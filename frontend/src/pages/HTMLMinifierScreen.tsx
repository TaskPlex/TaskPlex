...
export const HTMLMinifierScreen: React.FC = () => {
  const { t } = useTranslation();
  const [html, setHtml] = useState('');

  const { mutate, data: result, isPending: loading, error, reset } = useHTMLMinifier();
  ...
  const handleCopy = useCallback(() => {
    if (result?.minified_html) {
      navigator.clipboard.writeText(result.minified_html);
    }
  }, [result]);

  const handleDownload = useCallback(() => {
    if (result?.minified_html) {
      const blob = new Blob([result.minified_html], { type: 'text/html' });
    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="text-center mb-10">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2 flex items-center justify-center gap-3">
          ...
          {t('htmlMinifier.title')}
        </h1>
        <p className="text-gray-600 dark:text-gray-400">{t('htmlMinifier.description')}</p>
      </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <textarea
              ...
              placeholder={t('htmlMinifier.htmlPlaceholder')}
            ...
            <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
              {html.length} {t('htmlMinifier.characters')}
            </p>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center justify-between mb-4">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                {t('htmlMinifier.minifiedHTML')}
              </label>
              {result?.minified_html && (
            ...
            <textarea
              readOnly
              value={result?.minified_html || ''}
              placeholder={t('htmlMinifier.minifiedPlaceholder')}
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">
                      {t('htmlMinifier.originalSize')}:
                    </span>
                    <span className="ml-2 font-semibold text-gray-900 dark:text-white">
                      {result.original_length} {t('htmlMinifier.bytes')}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">
                      {t('htmlMinifier.minifiedSize')}:
                    </span>
                    <span className="ml-2 font-semibold text-gray-900 dark:text-white">
                      {result.minified_length} {t('htmlMinifier.bytes')}
                    </span>
                  </div>
                  {result.compression_ratio !== undefined && (
                    <div className="col-span-2">
                      <span className="text-gray-600 dark:text-gray-400">
                        {t('htmlMinifier.compressionRatio')}:
                      </span>
