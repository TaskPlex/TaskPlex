...
import { useCodeMinifier } from '../hooks/useCodeMinifier';

export const CodeMinifierScreen: React.FC = () => {
  const { t } = useTranslation();
  const [code, setCode] = useState('');
  const [language, setLanguage] = useState('auto');

  const { mutate, data: result, isPending: loading, error, reset } = useCodeMinifier();

  const handleMinify = useCallback(() => {
    if (!code.trim()) return;
    mutate({ code, language });
  }, [code, language, mutate]);

  const handleCopy = useCallback(() => {
    if (result?.minified_code) {
      navigator.clipboard.writeText(result.minified_code);
    }
  }, [result]);

  const handleDownload = useCallback(() => {
    if (result?.minified_code) {
      const ext = language === 'json' ? 'json' : language === 'xml' ? 'xml' : language === 'html' ? 'html' : language === 'css' ? 'css' : 'js';
      const blob = new Blob([result.minified_code], { type: 'text/plain' });
  ...
  const languages = [
    { value: 'auto', label: t('codeMinifier.language.auto') },
    { value: 'html', label: t('codeMinifier.language.html') },
    { value: 'css', label: t('codeMinifier.language.css') },
    { value: 'javascript', label: t('codeMinifier.language.javascript') },
    { value: 'json', label: t('codeMinifier.language.json') },
    { value: 'xml', label: t('codeMinifier.language.xml') },
  ];
    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="text-center mb-10">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2 flex items-center justify-center gap-3">
          ...
          {t('codeMinifier.title')}
        </h1>
        <p className="text-gray-600 dark:text-gray-400">{t('codeMinifier.description')}</p>
      </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <textarea
              ...
              placeholder={t('codeMinifier.codePlaceholder')}
            ...
            <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
              {code.length} {t('codeMinifier.characters')}
            </p>
          </div>
          ...
          >
            {loading ? (
              <>
                ...
                {t('codeMinifier.minifying')}
              </>
            ) : (
              <>
                <Minimize2 className="w-5 h-5" />
                {t('codeMinifier.minifyBtn')}
              </>
            )}
          </button>
        </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center justify-between mb-4">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                {t('codeMinifier.minifiedCode')}
              </label>
              {result?.minified_code && (
            ...
            <textarea
              readOnly
              value={result?.minified_code || ''}
              placeholder={t('codeMinifier.minifiedPlaceholder')}
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">
                      {t('codeMinifier.originalSize')}:
                    </span>
                    <span className="ml-2 font-semibold text-gray-900 dark:text-white">
                      {result.original_length} {t('codeMinifier.bytes')}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">
                      {t('codeMinifier.minifiedSize')}:
                    </span>
                    <span className="ml-2 font-semibold text-gray-900 dark:text-white">
                      {result.minified_length} {t('codeMinifier.bytes')}
                    </span>
                  </div>
                  {result.compression_ratio !== undefined && (
                    <div className="col-span-2">
                      <span className="text-gray-600 dark:text-gray-400">
                        {t('codeMinifier.compressionRatio')}:
                      </span>
