...
import { Lock, Unlock, Upload, Download, FileText, Eye, EyeOff } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { ApiService } from '../../services/api';
import { usePasswordPDF } from '../../hooks/usePDF';
import { useDownload } from '../../hooks/useDownload';

export const PDFPassword: React.FC = () => {
  ...
  const { mutate, isPending: loading, data: result, error, reset } = usePasswordPDF();
  ...
  const handleDownload = useCallback(() => {
    if (result?.download_url) {
      ...
      const filename = result.download_url.split('/').pop() || 'processed.pdf';
      ...
      <div className="text-center mb-10">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2 flex items-center justify-center gap-3">
          {action === 'add' ? (
            ...
            <Unlock className="text-blue-600 dark:text-blue-400" size={32} />
          )}
        ...
        {!file ? (
          <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-12 text-center hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors relative group cursor-pointer">
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
              {t('pdf.password.selectFile')}
            </h3>
            <p className="text-gray-500 dark:text-gray-400">
              {t('pdf.password.dropFile')}
            </p>
          </div>
        ) : (
          <div className="space-y-8">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-3 pr-12 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-red-500 focus:border-transparent"
                  />
                  <button
                    type="button"
                    ...
                    aria-label={showPassword ? t('pdf.password.hidePassword') : t('pdf.password.showPassword')}
                  >
                    {showPassword ? (
            ...
            {!result || !result.success ? (
              ...
              >
                {loading
                  ? t('pdf.password.processing')
                  : action === 'add'
                    ? t('pdf.password.addBtn')
                    : t('pdf.password.removeBtn')}
              </button>
            ) : (
              <div className="bg-green-50 dark:bg-green-900/30 border border-green-100 dark:border-green-800 rounded-xl p-6 text-center animate-in fade-in slide-in-from-bottom-4">
                ...
                <p className="text-green-700 dark:text-green-300 mb-6">
                  {result.message || t('pdf.password.successMessage')}
                </p>
                {result.download_url && (
                  <button
