...
import { RotateCw, ArrowRight, Video } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { ApiService } from '../services/api';
import { useRotateVideo } from '../hooks/useVideo';
import {
  FileDropzone,
  ProcessButton,
  ErrorAlert,
...
export const RotateVideoScreen: React.FC = () => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          <FileDropzone
            ...
            accept="video/*"
            fileType="video"
            labelKey="rotateVideo.videoFile"
            dropLabelKey="rotateVideo.dragDrop"
            color="blue"
          />
          ...
          <ProcessButton
            ...
            disabled={!file}
            loading={loading}
            labelKey="rotateVideo.rotateVideo"
            loadingLabelKey="rotateVideo.processing"
            color="blue"
          />
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                {t('rotateVideo.original')}
              </h3>
              <div className="bg-gray-100 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden aspect-video flex items-center justify-center relative">
                {previewUrl ? (
                  <video 
                    src={previewUrl} 
                    controls
              ...
              <div className="bg-gray-100 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden aspect-video flex items-center justify-center relative">
                {result?.download_url ? (
                  ...
                  <div className="text-gray-300 dark:text-gray-600 flex flex-col items-center">
                    ...
                    <span className="text-sm">{t('rotateVideo.preview')}</span>
                  </div>
                )}
                {result?.processed_size && (
                  <div className="absolute bottom-2 right-2 bg-green-600 text-white text-xs px-2 py-1 rounded font-bold shadow-sm">
                    {(result.processed_size / (1024 * 1024)).toFixed(2)} MB
                  </div>
                )}
              </div>
            </div>
          ...
          {result && result.success && (
            <ResultCard
              ...
              downloadLabelKey="rotateVideo.downloadResult"
              color="green"
            />
          )}
        </div>
