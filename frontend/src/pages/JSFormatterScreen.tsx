...
import { FileCode, Copy, Download, RefreshCw } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { useJSFormatter } from '../hooks/useJSFormatter';

export const JSFormatterScreen: React.FC = () => {
  ...
  const { mutate, data: result, isPending: loading, error, reset } = useJSFormatter();

  const handleFormat = useCallback(() => {
    if (!javascript.trim()) return;
    mutate({ javascript, indentSize, indentChar, wrapLineLength });
  }, [javascript, indentSize, indentChar, wrapLineLength, mutate]);

  const handleCopy = useCallback(() => {
    if (result?.formatted_js) {
      navigator.clipboard.writeText(result.formatted_js);
    }
  }, [result]);

  const handleDownload = useCallback(() => {
    if (result?.formatted_js) {
      const blob = new Blob([result.formatted_js], { type: 'text/javascript' });
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="text-center mb-10">
        ...
        <p className="text-gray-600 dark:text-gray-400">{t('jsFormatter.description')}</p>
      </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <textarea
              ...
              placeholder={t('jsFormatter.javascriptPlaceholder')}
            ...
            <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
              {javascript.length} {t('jsFormatter.characters')}
            </p>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full p-3 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-yellow-500 dark:focus:ring-yellow-400 focus:border-transparent text-gray-900 dark:text-white"
                >
                  <option value=" ">{t('jsFormatter.space')}</option>
                  <option value="\t">{t('jsFormatter.tab')}</option>
                </select>
              </div>
            </div>
            ...
            <div>
              <label
                ...
                className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2"
              >
                {t('jsFormatter.wrapLineLength')} ({wrapLineLength})
              </label>
              <input
                id="wrap-length"
                type="range"
              ...
              <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400 mt-1">
                <span>0 ({t('jsFormatter.noWrap')})</span>
                <span>200</span>
              </div>
            </div>
            ...
            <button
              ...
              disabled={loading || !javascript.trim()}
            ...
            >
              {loading ? (
                <>
                  ...
                  {t('jsFormatter.formatting')}
                </>
              ) : (
                <>
                  <FileCode className="w-5 h-5" />
                  {t('jsFormatter.formatBtn')}
                </>
              )}
            </button>
          </div>
